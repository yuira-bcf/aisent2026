...
export const creatorFlavorNotes = pgTable(
	"creator_flavor_notes",
	{
		...
		temperature: varchar("temperature", { length: 10 })
			.notNull()
			.$type<FlavorTemperature>(), // cool/neutral/warm
		memo: text("memo"),
		createdAt: timestamp("created_at").notNull().defaultNow(),
	},
	(table) => [
		index("idx_creator_flavor_notes_creator").on(table.creatorId),
		unique("uq_creator_flavor_notes_creator_flavor").on(
			table.creatorId,
			table.flavorId,
		),
	],
...
export const prohibitedReasonEnum = [
	"chemical_reaction",
	"odor_clash",
	"safety",
	"aesthetic",
...
export const prohibitedCombinations = pgTable(
	"prohibited_combinations",
	{
		...
		creatorId: uuid("creator_id")
			.notNull()
			.references(() => users.id),
		flavorAId: uuid("flavor_a_id")
			.notNull()
			.references(() => flavors.id),
		flavorBId: uuid("flavor_b_id")
			.notNull()
			.references(() => flavors.id),
	...
	(table) => [
		...
		unique("uq_prohibited_combinations_pair").on(
			table.creatorId,
			table.flavorAId,
			table.flavorBId,
		),
...
export const flavorCompatibility = pgTable(
	...
	(table) => [
		...
		unique("uq_flavor_compatibility_pair").on(
			table.creatorId,
			table.flavorAId,
			table.flavorBId,
		),
...
export const themeAxisTypeEnum = [
	"scene",
	"mood",
	"worldview",
	"relationship",
...
export const themeAxes = pgTable(
	"theme_axes",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		creatorId: uuid("creator_id")
	...
	(table) => [
		...
		unique("uq_theme_axes_creator_type_name").on(
			table.creatorId,
			table.type,
			table.name,
		),
...
export const profileVisitors = pgTable(
	"profile_visitors",
	{
		id: uuid("id").primaryKey().defaultRandom(),
		creatorId: uuid("creator_id")
...
export const creatorFlavorNotesRelations = relations(
	creatorFlavorNotes,
	({ one }) => ({
		creator: one(users, {
			fields: [creatorFlavorNotes.creatorId],
			references: [users.id],
		}),
		flavor: one(flavors, {
			fields: [creatorFlavorNotes.flavorId],
			references: [flavors.id],
		}),
	}),
);

export const prohibitedCombinationsRelations = relations(
	prohibitedCombinations,
	({ one }) => ({
		creator: one(users, {
			fields: [prohibitedCombinations.creatorId],
			references: [users.id],
		}),
		flavorA: one(flavors, {
			fields: [prohibitedCombinations.flavorAId],
			references: [flavors.id],
			relationName: "prohibitedFlavorA",
		}),
		flavorB: one(flavors, {
			fields: [prohibitedCombinations.flavorBId],
			references: [flavors.id],
			relationName: "prohibitedFlavorB",
		}),
	}),
);

export const flavorCompatibilityRelations = relations(
	flavorCompatibility,
	({ one }) => ({
		creator: one(users, {
			fields: [flavorCompatibility.creatorId],
			references: [users.id],
		}),
		flavorA: one(flavors, {
			fields: [flavorCompatibility.flavorAId],
			references: [flavors.id],
			relationName: "compatFlavorA",
		}),
		flavorB: one(flavors, {
			fields: [flavorCompatibility.flavorBId],
			references: [flavors.id],
			relationName: "compatFlavorB",
		}),
	}),
);
...
export const themeAxesRelations = relations(themeAxes, ({ one }) => ({
	creator: one(users, {
		fields: [themeAxes.creatorId],
		references: [users.id],
	}),
}));

export const profileVisitorsRelations = relations(
	profileVisitors,
	({ one }) => ({
		creator: one(users, {
			fields: [profileVisitors.creatorId],
			references: [users.id],
			relationName: "visitedCreator",
		}),
		visitor: one(users, {
