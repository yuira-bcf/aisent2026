import { pgTable, uuid, varchar, text, integer, boolean, decimal, timestamp, jsonb } from 'drizzle-orm/pg-core';

// ---------------------------------------------------------------------------
// AI Blend Rule types
// ---------------------------------------------------------------------------

export const ruleTypeEnum = ['weight_limit', 'ratio_constraint', 'compatibility', 'note_balance'] as const;
...
export const ecSettings = pgTable('ec_settings', {
  ...
  taxRate: decimal('tax_rate', { precision: 5, scale: 2 }).notNull().default('10.0'),
  freeShippingThresholdYen: integer('free_shipping_threshold_yen').notNull().default(5000),
...
export const calibrationCategoryEnum = ['blend_weight', 'note_ratio', 'keyword_influence', 'flavor_selection'] as const;
...
export const aiCalibrationParams = pgTable('ai_calibration_params', {
  ...
  description: text('description'),
  category: varchar('category', { length: 30 }).notNull().$type<CalibrationCategory>(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
