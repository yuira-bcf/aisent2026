import { pgTable, uuid, varchar, text, decimal, integer, boolean, timestamp, jsonb, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const creatorProfiles = pgTable('creator_profiles', {
  ...
  styleNoteBalance: jsonb('style_note_balance').$type<Record<string, number>>(),
  styleFlavorPreferences: jsonb('style_flavor_preferences').$type<Record<string, number>>(),
...
export const tierEnum = ['APPRENTICE', 'ARTISAN', 'EXPERT', 'MASTER', 'GRAND_MASTER'] as const;
export type Tier = (typeof tierEnum)[number];

export const creatorStats = pgTable('creator_stats', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').notNull().unique().references(() => users.id, { onDelete: 'cascade' }),
  tier: varchar('tier', { length: 20 }).notNull().default('APPRENTICE').$type<Tier>(),
  tierScore: decimal('tier_score', { precision: 10, scale: 2 }).notNull().default('0'),
  ...
  index('idx_creator_stats_user').on(table.userId),
]);
...
export const creatorFavorites = pgTable('creator_favorites', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  creatorId: uuid('creator_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => [
  uniqueIndex('idx_creator_favorites_user_creator').on(table.userId, table.creatorId),
...
export const creatorFavoritesRelations = relations(creatorFavorites, ({ one }) => ({
  user: one(users, { fields: [creatorFavorites.userId], references: [users.id] }),
  creator: one(users, { fields: [creatorFavorites.creatorId], references: [users.id] }),
}));
