...
export const recipeFlavors = pgTable(
	...
	(table) => [
		...
		unique("uq_recipe_flavors_recipe_flavor").on(
			table.recipeId,
			table.flavorId,
		),
	],
...
export const signatureRecipesRelations = relations(
	signatureRecipes,
	({ one, many }) => ({
		creator: one(users, {
			fields: [signatureRecipes.creatorId],
			references: [users.id],
		}),
