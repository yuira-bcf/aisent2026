...
export const keywordFlavorRules = pgTable('keyword_flavor_rules', {
  id: uuid('id').primaryKey().defaultRandom(),
  keywordId: uuid('keyword_id').notNull().references(() => keywords.id, { onDelete: 'cascade' }),
  flavorId: uuid('flavor_id').notNull().references(() => flavors.id, { onDelete: 'cascade' }),
...
export const keywordFlavorRulesRelations = relations(keywordFlavorRules, ({ one }) => ({
  keyword: one(keywords, { fields: [keywordFlavorRules.keywordId], references: [keywords.id] }),
  flavor: one(flavors, { fields: [keywordFlavorRules.flavorId], references: [flavors.id] }),
  creator: one(users, { fields: [keywordFlavorRules.creatorId], references: [users.id] }),
}));
